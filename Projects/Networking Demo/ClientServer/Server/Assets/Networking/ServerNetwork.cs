...
public class ServerNetwork : UCNetwork
{
    ...
    override protected void HandleMessage_Data(NetIncomingMessage aMsg)
    {
        ...
        if (type == MessageType.RPC)
        {
            ...
            MessageReciever receiver = (MessageReciever)aMsg.ReadUInt32();
            int networkId = aMsg.PeekInt32();

            // If this RPC includes server only, make that call first
            abortRPC = false;
            if (((int)receiver & (int)MessageReciever.ServerOnly) != 0)
            {
                RecieveRPC(aMsg);
            }
            ...
            if (!abortRPC)
            {
                if (((int)receiver & (int)MessageReciever.AllClients) != 0)
                {
                    sendMsg.Write(aMsg);
                    server.SendToAll(sendMsg, NetDeliveryMethod.ReliableOrdered);
                }
                else if (((int)receiver & (int)MessageReciever.AllClientsInArea) != 0)
                {
                    sendMsg.Write(aMsg);
                ...
                else if (((int)receiver & (int)MessageReciever.OtherClients) != 0)
                {
                    sendMsg.Write(aMsg);
                    server.SendToAll(sendMsg, aMsg.SenderConnection, NetDeliveryMethod.ReliableOrdered, 0);
                }
                else if (((int)receiver & (int)MessageReciever.OtherClientsInArea) != 0)
                {
                    sendMsg.Write(aMsg);
                ...
                else if (((int)receiver & (int)MessageReciever.ServerOnly) != 0)
                {
                    // If the message is sent only to the server, it should have already been recieved above
                }
                else
                {
                    Debug.LogError("RPC call sent to unknown MessageReciever type");
                }
            }
        ...
        else if (type == MessageType.SyncUpdate || type == MessageType.LiteSyncUpdate)
        {
            ...
            {
                //Position and rotation recieved
            ...
            if (receiver == MessageReciever.OtherClients)
            {
                server.SendToAll(sendMsg, aMsg.SenderConnection, NetDeliveryMethod.UnreliableSequenced, 0);

            }
            else if (receiver == MessageReciever.OtherClientsInArea)
            {
            ...
            {
                Debug.LogError("Unsupported MessageReciever sent to SyncUpdate: " + receiver);
            }
        ...
        else if (type == MessageType.VoiceData)
        {
            ...
            List<long> clientList = voipManager.GetClientsForVoiceChat(networkObjectId, (VoiceChatChannel)channel);
            ...
            if (clients.Count > 0)
            {
                ...
                globalMessage.Write((int)VoiceChatChannel.Radio);
        ...
        else if (type == MessageType.ConnectNetworkSync)
        {
            ...
            else
            {
                ...
                sendMsg.Write((int)MessageReciever.SingleClient);
                sendMsg.Write(networkId);
    ...
    */
    ...
    public void ConnectionApproved(long aClientId)
    {
       ...
       // database.GetPlayer(aClientId % 997);
    ...
    void SendInitializationData(ClientData aClient, int newAreaId)
    {
        ...
        foreach (KeyValuePair<int, NetworkObject> data in networkObjects)
        {
            ...
            sendMsg = server.CreateMessage();
            sendMsg.Write((int)MessageType.SyncUpdate);
            sendMsg.Write((int)MessageReciever.OtherClientsInArea); // This is ignored by the recieving client
            sendMsg.Write(data.Value.networkId);
            sendMsg.Write(data.Value.position.x);
            sendMsg.Write(data.Value.position.y);
            sendMsg.Write(data.Value.position.z);
    ...
    public override void SyncNetworkData(int aNetworkId, UCNetwork.MessageReciever aMessageReceiver, DeliveryMethod aDeliveryMethod, byte[] aData, Vector3 aPosition, Quaternion aRotation)
    {
        Debug.LogError("ServerNetwork::SyncNetworkData is not implemented!");
    }

    //Lite
    public override void LiteSyncNetworkData(int aNetworkId, UCNetwork.MessageReciever aMessageReceiver, DeliveryMethod aDeliveryMethod, byte[] aData)
    {
    ...
    public override void CallRPC(string aFunctionName, UCNetwork.MessageReciever aReciever, int aNetworkId, params object[] aParams)
    {
        if (aReciever == MessageReciever.OtherClientsInArea || aReciever == MessageReciever.AllClientsInArea)
        {
            Debug.LogError("ServerNetwork::CallRPC - Trying to call a RPC to area without an area id specified");
        }
        CallRPCToArea(aFunctionName, aReciever, null, aNetworkId, aParams);
    }

    // Used to call RPCs to an area
    public void CallRPCToArea(string aFunctionName, UCNetwork.MessageReciever aReciever, List<int> areaIds, int aNetworkId, params object[] aParams)
    {
        NetOutgoingMessage sendMsg = server.CreateMessage();
        sendMsg.Write((int)MessageType.RPC); // This is an rpc
        sendMsg.Write((int)MessageReciever.SingleClient); // Which other clients this should be sent to
        ...
        LogRPC(sendMsg, aReciever);

        switch (aReciever)
        {
            case MessageReciever.AllClients:
                server.SendToAll(sendMsg, NetDeliveryMethod.ReliableOrdered);
                break;

            case MessageReciever.AllClientsInArea:
                if (areaIds == null)
                {
                    Debug.LogError("ServerNetwork::CallRPCToArea - MessageReciever.AllClientsInArea: No area id specified.");
                    return;
                }
            ...
            case MessageReciever.OtherClients:
                Debug.LogError("ServerNetwork::CallRPCToArea - MessageReciever.OtherClients: Sending to OtherClients on server. Use AllClients instead.");
                break;

            case MessageReciever.OtherClientsInArea:
                if (areaIds == null)
                {
                    Debug.LogError("ServerNetwork::CallRPCToArea - MessageReciever.OtherClientsInArea: No area id specified.");
                    return;
                }
            ...
            case MessageReciever.ServerOnly:
                Debug.LogError("ServerNetwork::CallRPCToArea - MessageReciever.ServerOnly: Sending a RPC to the server from the server is unimplemented.");
                break;

            default:
                Debug.LogError("CallRPCToArea call sent to unknown MessageReciever type");
                break;
        }
    }
    ...
    public override void CallRPC(string aFunctionName, long aClientId, int aNetworkId, params object[] aParams)
    {
        ...
        LogRPC(sendMsg, MessageReciever.SingleClient);
    ...
    protected override void RecieveRPC(NetIncomingMessage aMsg)
    {
        int networkId = aMsg.ReadInt32();
    ...
    // Log an outgoing RPC
    void LogRPC(NetOutgoingMessage aMsg, UCNetwork.MessageReciever aReciever)
    {
        if (logFile == "" && writeToLog)
        {
            writeToLog = false;
        ...
        // RPC Reciever Id:id Function:SomeFunc( arg, arg, arg, arg )
        string logMessage = "=> Outgoing " + (MessageType)aMsg.ReadInt32() + " Sent to: " + aReciever + " as " + (MessageReciever)aMsg.ReadUInt32();
    ...
    void LogRPC(NetIncomingMessage aMsg)
    {
        ...
        // RPC Reciever Id:id Function:SomeFunc( arg, arg, arg, arg )
        string logMessage = "RPC " + (MessageReciever)aMsg.ReadUInt32();
        logMessage += " NetSyncId: " + aMsg.ReadInt32();
        string temp = "";
        aMsg.ReadString(out temp);
