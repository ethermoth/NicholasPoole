...
public abstract class UCNetwork : MonoBehaviour
{
    ...
    abstract public void CallRPC(string aFunctionName, MessageReciever aReciever, int aNetworkId, params object[] aParams);
    abstract protected void RecieveRPC(NetIncomingMessage aMsg);
    ...
    // Sync network data
    abstract public void SyncNetworkData(int aNetworkId, MessageReciever aMessageReceiver, DeliveryMethod aDeliveryMethod, byte[] aData, Vector3 aPosition, Quaternion aRotation);

    // Lite Sync netword data
    abstract public void LiteSyncNetworkData(int aNetworkId, MessageReciever aMessageReceiver, DeliveryMethod aDeliveryMethod, byte[] aData);
