   ...
   List<ClientData> clientData = new List<ClientData>();
    ...
    void SyncObjectsToClients() {
        ...
        foreach (KeyValuePair<int, ObjectSyncData> objectSync in netObjsToSync) {
            ...
            sendMsg.Write((int)objectSync.Value.reciever);
            ...
            // Who should this update be sent to
            if (objectSync.Value.reciever == MessageReciever.OtherClients) {
                server.SendToAll(sendMsg, GetClientData(objectSync.Value.client).connection, NetDeliveryMethod.ReliableSequenced, 0);

            } else if (objectSync.Value.reciever == MessageReciever.OtherClientsInArea) {
                List<NetConnection> clients = GetClientsInArea(objectSync.Value.client, false);
                if (clients.Count > 0) {
                    server.SendMessage(sendMsg, clients, NetDeliveryMethod.ReliableSequenced, 0);
                }
            } else {
                Debug.LogError("Unsupported MessageReciever sent to SyncUpdate: " + objectSync.Value.reciever);
            }
        }
        netObjsToSync.Clear();
    }
       ...
       void LiteSyncObjectsToClients() {
        ...
        foreach (KeyValuePair<int, ObjectSyncData> objectSync in netObjsToSync) {
            ...
            if (objectSync.Value.reciever == MessageReciever.OtherClients) {
                ...
                Debug.LogError("Unsupported MessageReciever sent to LiteSyncUpdate: " + objectSync.Value.reciever);
            }
        }
    }   
    ...
    public override void CallRPC(string aFunctionName, UCNetwork.MessageReciever aReciever, int aNetworkId, params object[] aParams) {
        CallRPCToArea(aFunctionName, aReciever, -1, aNetworkId, aParams);
    }

    // Used to call RPCs to an area
    public void CallRPCToArea(string aFunctionName, UCNetwork.MessageReciever aReciever, int aAreaId, int aNetworkId, params object[] aParams) {
        NetOutgoingMessage sendMsg = server.CreateMessage();
