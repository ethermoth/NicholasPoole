...
public class ClientNetwork : UCNetwork
{
    ...
    Dictionary<int, VoiceChatPlayer> remoteVoiceChatPlayers;
    ...
    override protected void Awake()
    {
        ...
        remoteVoiceChatPlayers = new Dictionary<int, VoiceChatPlayer>();
    ...
    override protected void HandleMessage_Data(NetIncomingMessage aMsg)
    {
        ...
        {
            aMsg.ReadInt32(); // Read the reciever value, which we don't care about here
            ...
            {
                Debug.LogWarning("Recieved a SyncUpdate for a networkId this client doesn't know about: " + networkId);
                return;
            }
            if (type == MessageType.SyncUpdate)
            {
                netIdObjMap[networkId].networkSync.RecieveSyncData(byteData, position, orientation);
            }
            else if (type == MessageType.LiteSyncUpdate)
            {
                netIdObjMap[networkId].networkSync.RecieveLiteSyncData(byteData);
            }
        }
        ...
        else if (type == MessageType.ConnectNetworkSync)
        {
            ...
            */
            ...
            Debug.LogError("ConnectNetworkSync recieved, but was unable to find an unconnected network sync.");
        }
        ...
        else if (type == MessageType.VoiceData)
        {
            ...
            switch (channel)
            {
                ...
                case VoiceChatChannel.Proximity:
                    ...
                    {
                        Debug.Log("Proximity VoiceData has been recieved by this client, but it doesn't have a network object with id " + networkObjectId);
                    }
                    break;
            }
        }
    ...
    override public void CallRPC(string aFunctionName, MessageReciever aReciever, int aNetworkId, params object[] aParams)
    {
        CallRPC(aFunctionName, aReciever, aNetworkId, NetDeliveryMethod.ReliableOrdered, aParams);
    }

    // Call a RPC with a specific net delivery method
    public void CallRPC(string aFunctionName, MessageReciever aReciever, int aNetworkId, NetDeliveryMethod aDeliveryMethod, params object[] aParams)
    {
        if (!IsConnected())
        {
        ...
        sendMsg.Write((int)aReciever); // Which other clients this should be sent to
    ...
    override public void SyncNetworkData(int aNetworkId, MessageReciever aMessageReceiver, UCNetwork.DeliveryMethod aDeliveryMethod, byte[] aData, Vector3 aPosition, Quaternion aRotation)
    {
        if (!IsConnected())
        {
            return;
    ...
    override public void LiteSyncNetworkData(int aNetworkId, MessageReciever aMessageReceiver, UCNetwork.DeliveryMethod aDeliveryMethod, byte[] aData)
    {
        if (!IsConnected())
        {
            return;
