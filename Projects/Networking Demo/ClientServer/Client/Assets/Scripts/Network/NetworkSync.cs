...
public class NetworkSync : MonoBehaviour
{
    ...
    //Ensure that non-client players will appear as expected
    ...
    void Update()
    {
        ...
        if (this.GetComponent<Player>() == null && this.GetComponent<ItemIdentifier>() == null)
        {
            if(this.GetComponent<NetworkInterpolatedTransform>().isActiveAndEnabled == false)
            {
    ...
    {
        clientNet.CallRPC("SetFollowsClient", UCNetwork.MessageReciever.ServerOnly, -1, networkId, aShouldFollow);
    ...
    public void CallRPC(string aFunctionName, UCNetwork.MessageReciever aReciever, params object[] aParams)
    {
        /*
        int sendingNetworkId = networkId;
        if (aReciever == UCNetwork.MessageReciever.ServerOnly) {
            sendingNetworkId = -1;
        }
        */
        if (networkId == -1)
        ...
        if (clientNet)
        {
            clientNet.CallRPC(aFunctionName, aReciever, networkId, aParams);
        }
    }
    ...
    //////////////////
    // Recieve a sync
    public void RecieveSyncData(byte[] aData, Vector3 aPosition, Quaternion aRotation)
    {
#if UNITY_EDITOR
    ...
    //Recieve a (lite) sync
    public void RecieveLiteSyncData(byte[] aData)
    {
        MemoryStream stream = new MemoryStream(aData);
        BinaryReader binReader = new BinaryReader(stream);
        if (setDataFunction != null)
    ...
    void BroadcastSync()
    {
        ...
        if (position != tPosition || rotation != tRotation)
        {
            ...
            if (inAreaOnly)
            {
                clientNet.SyncNetworkData(networkId, UCNetwork.MessageReciever.OtherClientsInArea, deliveryMethod, dataArray, position, rotation);
            }
            else
            {
                clientNet.SyncNetworkData(networkId, UCNetwork.MessageReciever.OtherClients, deliveryMethod, dataArray, position, rotation);
            }
        ...
        else if (arrayDataChanged)
        {
            ...
            if (inAreaOnly)
            {
                clientNet.LiteSyncNetworkData(networkId, UCNetwork.MessageReciever.OtherClientsInArea, deliveryMethod, dataArray);
            }
            else
            {
                clientNet.LiteSyncNetworkData(networkId, UCNetwork.MessageReciever.OtherClients, deliveryMethod, dataArray);
            }
        }
        else
        {
